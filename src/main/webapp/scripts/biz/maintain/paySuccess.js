...
var orderSucDetail = {
    init : function() {
        ...
        //var tradeid="1609050018219576";
        orderSucDetail.orderInfo(tradeid);
    },
...
//						com.skyform.service.callAsyncApi4yaxin("/trade/queryTreadInfoById", datas, function(data) {
                if (data.code != "0") {
                    ...
                    if(data != null){
                        ...
                        $("#tradeType").html(orderSucDetail.getOrderStatus(tradeType));
                        //时间显示
...
//									//TODO:数据虚拟
                        ...
                        orderSucDetail.getConfigSpecification(data.data.tradeDetailInfo,0,serviceType,tradeType);
...
//									//TODO:数据虚拟
    ...
    getConfigSpecification : function(templateData,billType,serviceType,tradeType) {
        ...
        if(tradeType == 'A'){//购买订单
            switch (serviceType) {
                case 1001://弹性云主机
                    value = orderSucDetail.showAVMDetail(templateData,billType,"vm");
                    break;
                case 1003://弹性负载均衡
                    value = orderSucDetail.showABlanceDetail(templateData);
                    break;
                case 1002://弹性块存储
                    value = orderSucDetail.showAVdiskDetail(templateData);
                    break;
                case 1006://公网带宽
                    value = orderSucDetail.showAIpDetail(templateData);
                    break;
                case 1013://文件存储
                    value = orderSucDetail.showANasDetail(templateData);
                    break;
                case 1012://对象存储
                    value = orderSucDetail.showAObsDetail(templateData);
                    break;
                case 1008://路由器
                    value = orderSucDetail.showARouteDetail(templateData);
                    break;
                case 1009://虚拟私有云
                    value = orderSucDetail.showASubnetDetail(templateData);
                    break;
                case 1017:
                    $("#productName").html(snapText);
                    value = orderSucDetail.showASnapDetail(templateData);
                    break;
                case 1018:
                    $("#productName").html(imageText);
                    value = orderSucDetail.showAImageDetail(templateData);
                    break;
            }
        ...
        if(value != ""){
            ...
            if(specarr){
                ...
                //});
                $("#cms").html($(specarr)[0].split(':')[1]+' '+$(specarr)[1].split('：')[1]);
                $("#osMemory").html($(specarr)[2].split('：')[1]);
                if($(specarr)[5]){
                    $("#dataMemory").html($(specarr)[5].split('：')[1]);
                }
                $("#bandWidth").html($(specarr)[3].split('：')[1]);
            }
        }
