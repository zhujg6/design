...
//lb_fan 基础类跳转过来之后的操作
function  basicChanges(){
	//lb_fan
	var codeS=GetQueryString("code");
	var imageS=GetQueryString("image");
	var osTypeS=GetQueryString("osType");
	var image_div_id=GetQueryString("image_div_id");
	var image_div_id_val=$("#productNameInput").val();
	if(codeS==1){
		//镜像
		$("#image").find("div").removeClass("active");
		$("#image").find("div[value="+imageS+"]").addClass("active");
		$("#osType").find("div").removeClass("active");
		$("#osType").find("div[value="+osTypeS+"]").addClass("active");
		$("#mirrorName").html(image_div_id_val);
		var str='<option value="'+image_div_id+'">'+image_div_id_val+'</option>';
		$("#os_div").find("select").empty().html(str)
		//$("#os_div").find("option").html(image_div_id_val).attr("value",);
		$("#_os").empty().html(image_div_id_val).show();
		$(".lb_fan div div").hover(function(){
			if(!$(this).hasClass("active")){
				$(this).attr("style","background-color:#e8e8e8;color:#666;");
			}
		});
	}
	if(!$("#pool").find("div").hasClass("active")){
		//console.log($('#pool div[data-value="xiank"]').html())
		if($("#currentPool").val() =="xiank"){
			$('#pool div[data-value="xiank"]').addClass("active");
		}else if($("#currentPool").val() =="shanghai"){
			$('#pool div[data-value="shanghai"]').addClass("active");
		}else{
			$('#pool div[data-value="xiank"]').addClass("active");
		}
	}
...
function listResourcePool(offLineBill) {//资源池列表信息
	...
	poolContainer.empty();
	//公有云所有的资源池 common.json --  publicregisterpools
	var strpools = Config.publicregisterpools;
	//var strpools = {shanghai: "上海", xiank: "西咸基地A",langfangk:"廊坊jdc"};
	var pools = [];
	var strpoolsFilter={};
	var _pooldata="";
	if ($.trim(strpools) != "" && strpools != null && strpools != "null") {
		//lb_fan 过滤资源池 只有xiank  shanghai  langfangk 才能购买（页面显示）
		//当前用户的资源池
		//var arr=$("#resourcePools").val().split(",");
		//if(arr[arr.length-1] == ""){
		//	delete arr[arr.length-1];
		//}
		//for(var i=0;i<arr.length;i++){
		//	if(arr[i]=="xiank"){
		//		strpoolsFilter.xiank="西北"
		//	}
		//	if(arr[i]=="shanghai"){
		//		strpoolsFilter.shanghai="华东"
		//	}
		//}
		strpoolsFilter.xiank="西北";
		strpoolsFilter.shanghai="华东";
		//pools = strpools;
		pools = strpoolsFilter;
				...
				if(item.displaytext=="小型A"){
					tab_option
						.click(function() {
							...
							if($("#_day").html()=="1年"&&$("#pool .active").attr("data-value")!="huhehaote"){
								$("#agentId").val('');
								$("#agentId").attr('disabled','disabled');
							}else if($("#_day").html()=="2年"&&$("#pool .active").attr("data-value")!="huhehaote"){
								$("#agentId").val('');
								$("#agentId").attr('disabled','disabled');
							}
							else if($("#_day").html()=="3年"&&$("#pool .active").attr("data-value")!="huhehaote"){
								$("#agentId").val('');
								$("#agentId").attr('disabled','disabled');
							}
							else{
...
function write_vmfee(){
	...
	if($("#pool .active").attr("data-value")!="huhehaote"){
		$("#vm-ul tr").eq(0).find(".vm_fee").html("<span style='color: #f00;'>40.7元</span>/月起  <img src='../../images/dobule11_365.png' title='双十一优惠' style='width: 60px;width: 55px\0;*width: 55px;vertical-align: text-top;'/>");
	}
	else{
...
function _initWizard() {
	...
	$("#payPeriod").mouseup(function(){
		...
		if(ui_value<10){
			...
			//双十一star
			if(ui_value==10&&$("#vm-ul .active").attr('data-value')=="小型A"){
				if($("#pool .active").attr("data-value")!="huhehaote"){
					$("#agentId").val('');
					$("#agentId").attr('disabled','disabled');
				}
			}else if(ui_value==11&&$("#vm-ul .active").attr('data-value')=="小型A"){
				$("#agentId").val('');
				$("#agentId").attr('disabled','disabled');
			}
			else if(ui_value==12&&$("#vm-ul .active").attr('data-value')=="小型A"){
				$("#agentId").val('');
				$("#agentId").attr('disabled','disabled');
			}
			else{
	...
	$("#buy").unbind("click").bind("click",function() {
		console.log(osDisk_=GetQueryString("osDisk"))
		if(validateAgentCodeCheck()==false){return;}
		$.blockUI();
		$('.blockOverlay').click($.unblockUI);
		if (currentUserId == 0) {
			var url = ctx + "/jsp/login.jsp?returnURL=createVMfan";
		...
		//在这把instance.productList[0].osDisk重新赋值（$("#imagetemplates").val()）
		//console.log(instance.productList[0].OS);
		instance.productList[0].osDisk=osDisk_
		//console.log(instance.productList[0].osDisk);
				...
				function onCreated(data) {
					...
					com.skyform.service.BillService.wizardConfirmTradeSubmit2(disCountFee,_tradeId,null,
						function onSuccess(data) {
							...
							window.location.href ="../../jsp/maintain/paySuccess.jsp?code=order&cataCode=consume&tradeId="+_tradeId+'&osName='+encodeURIComponent($("#ostemplates").text());
						},
