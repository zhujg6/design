...
//			else $("#openMore").empty().html("展开");
		...
		$("#moreAction").addClass("disaController bled");
...
//					else 
				...
				if(to == 6){
					...
					var config = vmJson.getVM4JsonInstanceForVps();
					var obj = {	};
					...
					if(mirrorType=="using" || mirrorType=="all"){
						...
						obj.image = $("#ostemplates .osList.sesnapCountlected span").text();
...
//					});
			...
			VM.wizard.onFinish = function(from, to) {
				...
				if($("#diy_tab").hasClass("active")){
					if($("#basicNetwork-tab").hasClass("active")){
						if($(".cpu-options").attr("data-value")>0 && $("#period input").val()>0 && $(".memory-options").attr("data-value")>0){
							console.log("cup+cuo")
							VM.createVMPost(VM.wizard);
						}
					}else if ($("#privateNetwork-tab").hasClass("active")){
						if($(".cpu-options").attr("data-value")>0 && $("#period input").val()>0 && $(".memory-options").attr("data-value")>0
								&& $("#acreateBandwidthSize").val()>=1){
							VM.createVMPost(VM.wizard);
						}
					}
				}else {
					if($("#basicNetwork-tab").hasClass("active")){
						if( $("#memory").html()>0 && $("#cpu").html()>0 && $("#period input").val()>0 &&
							$("#createDataDiskSize").val()>=0){
							VM.createVMPost(VM.wizard);
						}
					}else if($("#privateNetwork-tab").hasClass("active")) {
						console.log("cup+cuo")
						if( $("#memory").html()>0 && $("#cpu").html()>0 && $("#period input").val()>0
								&& $("#acreateBandwidthSize").val()>=1 && $("#createDataDiskSize").val()>=0){
							console.log("cup+cuo")
							VM.createVMPost(VM.wizard);
						}
					}
...
//	创建虚拟机
	createVMPost : function(wizard) {
		var instance = vmJson.getVM4JsonInstanceForVps();
			...
			$(instance.productList).each(function(i,item){
				...
				if(item.cpuNum){
					...
					if($("#privateNetwork").hasClass("active")){
						...
						var itemFee_net=itemFee;
						itemFee_net.productPropertyList.push({
...
//						        		   return "恢复中";
									...
									//vps
						        	   ...
						        	   try {
										   ...
										   if(columnData.networkType =="1"){
											   var appendText = "CPU:"
												   + cpuNum + Dict.val("vm_he")+","+Dict.val("dc_ram")+":"
												   + mem + "G,"
												   + os +",带宽:"+columnData.bandwidth+"M";
...
//			});
	...
	checkSelected : function(selectInstance) {
		...
		var allCheckedBox = $("#tbody2 input[type='checkbox']:checked");
		var nettype=$(allCheckedBox).parents("tr").children("td").eq(4).html()
		if(nettype=="私有网络"){
			$("#bandwidthVM").addClass("disabled");
			$("#bandwidthVM1").addClass("disabled");
			$("#bandwidthVM").unbind("click");
			$("#bandwidthVM1").unbind("click");
		}else {
			$("#bandwidthVM").removeClass("disabled");
			$("#bandwidthVM1").removeClass("disabled");
			$("#bandwidthVM").bind("click");
			$("#bandwidthVM1").bind("click");
		}
		if(rightClicked) {
						...
						buttons : [
								{
									...
									onClick : function() {
										...
										VM.service.changeVMOs(changeos,function(){
											...
											$.growlUI(Dict.val("common_tip"),Dict.val("vm_operating_system_successfully"));
											VM.describleVM();
										},function(errorMsg){
						...
						buttons : [
								{
									...
									onClick : function() {
										...
										if(Number(cpu) >0 && Number(memory)>0){
...
//												$("#createModal").modal("hide");
											...
											$.growlUI(Dict.val("common_tip"), "参数不合法!");
										}
								...
								beforeShow : function(){
									...
									var osDes = VM.getCheckedArr().parents("tr").attr("os")?VM.getCheckedArr().parents("tr").attr("os"):"CENT";
									if(!VM.isWin){
										if((osDes.toLowerCase()).indexOf("win")!=-1){
											VM.isWin = true;
										}
		...
		if(VM.selectedInstanceId>0&&VM.selectedInstanceOs.length>0){
			...
			if (data.code != "0") {
				...
				confirmModal =  new com.skyform.component.Modal(
							...
							"<h4>"+Dict.val("vm_reset_current_operating_system")+VM.selectedInstanceOs+ "</h4><span id='snapCount' style='color:rgb(46, 46, 46);font-size:12px;'></span>",
							{
								buttons : [
										{
											name : Dict.val("common_determine"),
										...
										afterShow : function(){
											if(data.snaps){
												if(data.snaps.length > 0){
													...
													if(ss.length>0){
														$("#snapCount").empty().html("<i class='icon-info-sign' style='font-size: 14px'></i>本操作将使此云主机下所有快照失效，请确定？");
													}
												}
											}else{
												$("#snapCount").hide();
	...
	showRadomPassword:function(){
		...
		VM.service.getVmRandomPassword(condition,function(result){
			...
			var confirmModal = new com.skyform.component.Modal(
					...
					"<h4>"+Dict.val("vm_initial_password")+newPassword+ "</h4><br><h5>"+Dict.val("vm_reset_your_system_password")+"</h5>",
					{
						buttons : [
								{
									name : Dict.val("common_determine"),
					...
					buttons : [
						{
							...
							onClick : function() {
								...
								var oldBandWidth=$("#oldBandWidth").html();
								console.log(limitRate+","+limitRate)
								if (Number(limitRate) <= oldBandWidth) {
									...
									var muprty = {
										"bandwidth" : Number(limitRate)
									};
									var allCheckedBox = $("#tbody2 input[type='checkbox']:checked");
									var subId= Number($(allCheckedBox[0]).parents("tr").attr("id"));
									com.skyform.service.PortalInstanceService.modprodprty("serviceroffering",
											subId, muprty, function(data) {
													// 订单提交成功后校验用户余额是否不足
													var _tradeId = data[0].tradeId;
													var _fee = data[0].feeValue / 1000;
													var _modal = $("#id_bandwidth");
...
//		$("#tipName_confirmPassword").removeClass("error").empty();
		...
		{
			$("#tipName_nPassword").text(Dict.val("tenant_8_to_32_password"));
			flag = false;
		}
...
//		}
		if (_nPassword != _confirmPassword) {
			$("#tipName_confirmPassword").text(Dict.val("user_confirm_password_and_password_inconsistent"));
			flag = false;
		}
		if(flag == false)
			return;
